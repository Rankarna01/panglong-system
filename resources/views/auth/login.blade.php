...
<head>
    ...
    <title>Login - {{ isset($global_setting) ? $global_setting->app_name : 'Sistem Panglong' }}</title>
...
<body class="bg-surface min-h-screen text-slate-800 selection:bg-primary selection:text-white">
    ...
    <div class="flex min-h-screen w-full">
        ...
        <div class="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center overflow-hidden bg-white p-12 border-r border-slate-100">
            ...
            <div class="w-full max-w-lg mb-8 fade-in flex justify-center">
                @if(isset($global_setting) && $global_setting->logo_path)
                    <img src="{{ asset('storage/' . $global_setting->logo_path) }}" alt="Logo" class="w-64 h-64 object-contain drop-shadow-xl">
                @else
                    <div class="w-64 h-64 rounded-full bg-primary/10 text-primary flex items-center justify-center drop-shadow-xl">
            ...
            <div class="relative z-10 max-w-lg text-center fade-in" style="animation-delay: 0.2s;">
                <h1 class="text-4xl font-extrabold text-slate-800 mb-4 tracking-tight uppercase">{{ isset($global_setting) ? $global_setting->app_name : 'Sistem Panglong' }}</h1>
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-center bg-surface relative p-6 sm:p-12">
            ...
            <div class="max-w-md w-full bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8 sm:p-10 relative z-10 fade-in" style="animation-delay: 0.3s;">
                ...
                <div class="text-center mb-10">
                    <div class="lg:hidden w-20 h-20 rounded-2xl bg-white shadow-sm border border-slate-100 flex items-center justify-center mx-auto mb-6 overflow-hidden">
                        @if(isset($global_setting) && $global_setting->logo_path)
                            <img src="{{ asset('storage/' . $global_setting->logo_path) }}" alt="Logo" class="w-full h-full object-contain p-2">
                        @else
                            <i class="fas fa-warehouse text-3xl text-primary"></i>
                        @endif
                    </div>
